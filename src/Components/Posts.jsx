...
const Posts = ({ allPage, filterCategory, active ,unActive }) => {
  ...
  return (
    <div className="">
      {allFilter.length > 0 ?
        <div className="">
          ...
          <div className="pt-[10px]">
            {showAll ? filterCategory.length > 5 &&
              <div onClick={handleShow} className="w-[120px] rounded-[10px] text-center flex items-center gap-2 px-6 py-3 text-white font-semibold text-baserounded-lg bg-gradient-to-r from-[#000000] to-[#043a9770]overflow-hidden  transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]  hover:translate-y-[-2px] hover:rounded-[20px] hover:shadow-[0_5px_15px_rgba(106,17,203,0.3)]dark:from-[#180f25] dark:to-[#3a7bd5] dark:hover:shadow-[0_5px_15px_rgba(138,62,255,0.3)]cursor-pointer">
                <button>Show All</button>
              </div>
              :
              <div onClick={handleLess} className="w-[130px] rounded-[10px] text-center flex items-center gap-2 px-6 py-3 text-white font-semibold text-baserounded-lg bg-gradient-to-r from-[#000000] to-[#043a9770]overflow-hidden  transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]  hover:translate-y-[-2px] hover:rounded-[20px] hover:shadow-[0_5px_15px_rgba(106,17,203,0.3)]dark:from-[#180f25] dark:to-[#3a7bd5] dark:hover:shadow-[0_5px_15px_rgba(138,62,255,0.3)]cursor-pointer">
        ...
        : (
          <div className={` ${active ? '': 'grid  md:grid-cols-3  gap-4'}`} >
            {allPage.map((item) => (
              <div className="">
                <div className="h-[400px]" key={item.id}>
                <div className={` ${active ? 'relative flex h-[250px] items-center justify-around overflow-hidden shadow-md group'  : 'relative h-full overflow-hidden shadow-md group' } `}>
                  <Link to={`/Shop/${item.id}`}>

                    <img className={ `${active ? 'w-[80%] cursor-pointer object-cover hover:bg-[#f0f8ff]': 'w-full cursor-pointer object-cover hover:bg-[#f0f8ff]'}`} src={item.thumbnail} alt="" />
                  </Link>
                  <div className="absolute top-4 left-3 z-5">
                    <p className="py-[5px] px-[20px] border bg-black text-white font-bold text-[14px] font-dm">
                      {item.discountPercentage}%
                  ...
                  <div className={`${active ? 'absolute left-0 bottom-0 w-full  hidden group-hover:block   text-center items-center gap-2 px-6 py-3 text-white font-semibold text-baserounded-lg bg-gradient-to-r from-[#000000] to-[#043a9770]overflow-hidden  transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]  hover:translate-y-[-2px]  hover:shadow-[0_5px_15px_rgba(106,17,203,0.3)]dark:from-[#180f25] dark:to-[#3a7bd5] dark:hover:shadow-[0_5px_15px_rgba(138,62,255,0.3)]cursor-pointer' : "absolute right-0 bottom-25 w-[350px] bg-white hidden group-hover:block"}`}>
                    <div>
                      <div className={`${active ? 'flex items-center gap-2 justify-end  cursor-pointer' : 'flex items-center gap-2 justify-end py-[10px] pr-[10px] cursor-pointer'}`}>
                        
                        <p>Add to Wish List</p>
                        <GiSelfLove />
                      </div>
                      <div className={`${active ? 'flex items-center gap-2 justify-end py-[10px] cursor-pointer' : 'flex items-center gap-2 justify-end py-[10px] pr-[10px] cursor-pointer'}`}>
                        <TfiReload />
                      </div>
                      <div className={`${active ? 'flex items-center gap-2 justify-end  cursor-pointer' : 'flex items-center gap-2 justify-end py-[10px] pr-[10px] cursor-pointer'}`}>
                        <p>Add to Cart</p>
                  ...
                  <div className={`${active ? 'justify-between pl-[20px] pr-[20px] pt-2' : 'flex justify-between pl-[20px] pr-[20px] pt-2'}`}>
                    <div>
                      <p className="font-bold font-dm text-[16px] text-[#262626]">{item.title}</p>
                      <p className="font-bold font-dm text-[16px] text-[#8a8686] ">{item.category}</p>
                    </div>
